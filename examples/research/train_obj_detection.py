...
from fline.models.models.research.history.object_detection_model import (
    ObjectDetectionModelSMP,
)
from fline.losses.research.bbox import BboxLossFixed
...
TASK_NAME = 'smp_fpn_from_mask_reshaped_not_fixed_loss'
SEG_MODEL = 'seg_model'
MASK = 'mask'
BBOX = 'bbox'
DEVICE = 'cuda:3'
...
def generate_get_bboxes(mode='train'):
    def get_bboxes(row, res_dict):
        ...
        mask = mask[:, :, 2:3]
        r_mask = np.zeros((*SHAPE, 1), dtype='int')
        last_i = 1
        bboxes = []
        #print('-'*60)
        for i, m in enumerate(np.unique(mask)):
